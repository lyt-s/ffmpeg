...
// decoder_decode_frame() 函数的逻辑就是从解码器读取到 一个 AVFrame，为了解码出一个AVFrame，
// 它会从 PacketQueue 队列取 AVPacekt 发送给解码器，需要多少个就取多少个 AVPacekt，直至到能解码出一个 AVFrame。
static int decoder_decode_frame(Decoder *d, AVFrame *frame, AVSubtitle *sub) {
    ...
    for (;;) {
        ...
        do {
            ...
            // 如果没有数据可读了，就需要唤醒 read_thread() 线程来读数据？？
            ...
            if (d->packet_pending) {
                ...
                // 从 队列读取 AVPacket
                // 如果 PacketQueue 队列里面如果没有数据可读，可以一直阻塞等到 read_thread() 线程读到数据放进去队列 为止。
            ...
            //  PackeQueue 队列之前缓存的 5 帧数据就不可用了。丢弃这 5 帧数据就是由下面的代码实现的。
        ...
        if (d->avctx->codec_type == AVMEDIA_TYPE_SUBTITLE) {
            ...
            // 判断之前发送 AVPacket 给解码器是否失败了？如果失败，d->packet_peding 会是 1。
            // 如果上次失败了，d->pkt本身就是有值的，就不需要重队列里面拿数据，直接把 d->pkt 发送给解码器即可。
...
static int audio_thread(void *arg)
{
    ...
    do { // 从 PacketQueue 里面解码出来 AVFrame，此函数会阻塞，直到解码出来 AVFrame，或者返回错误。这个函数有 3 个返回值。
        if ((got_frame = decoder_decode_frame(&is->auddec, frame, NULL)) < 0)
            goto the_end;

        if (got_frame) {
                tb = (AVRational){1, frame->sample_rate};
                // 如果解码出来的 AVFrame 的音频格式与入口滤镜要求的音频格式不一样，会重建滤镜
                reconfigure =
            ...
            while ((ret = av_buffersink_get_frame_flags(is->out_audio_filter, frame, 0)) >= 0) {
                ...
                // Frame->serial = Decode->serial
                af->serial = is->auddec.pkt_serial;
                af->duration = av_q2d((AVRational){frame->nb_samples, frame->sample_rate});

                av_frame_move_ref(af->frame, frame);
                // 把 AVFrame 插入 FrameQueue 队列。
...
static int video_thread(void *arg)
{
    ...
    // 上一次解码出来的 AVFrame 的宽度，初始值为 0
    int last_w = 0;
    // 上一次解码出来的 AVFrame 的高度，初始值为 0
    int last_h = 0;
    // 上一次解码出来的 AVFrame 的像素格式，初始值为 -2
    enum AVPixelFormat last_format = -2;
    // 上一次解码出来的 AVFrame 的序列号，初始值为 -1
    int last_serial = -1;
...
static void sdl_audio_callback(void *opaque, Uint8 *stream, int len)
{
    ...
    // len --- 此次回调需要写多少字节的数据进去 stream 指针。
    while (len > 0) {
        if (is->audio_buf_index >= is->audio_buf_size) {
            // audio_decode_frame() 函数就是负责从 FrameQueue 读取 AVFrame，然后 把 is->audio_buf 指针指向 AVFrame 的 data
            // 返回值代表可以传输的音频数据有多少字节。
        ...
        // 可以传输的音频数据有多大，有多少个字节。
        // 还剩下多少数据没有传输过去--->将要传输的数据
        len1 = is->audio_buf_size - is->audio_buf_index;
        if (len1 > len)
            len1 = len;
        //  is->audio_volume == SDL_MIX_MAXVOLUME 最大音量
        if (!is->muted && is->audio_buf && is->audio_volume == SDL_MIX_MAXVOLUME)
        //  stream 这个指针是 SDL 内部音频数据内存的指针，只要把数据拷贝到这个指针的地址，就能播放声音了。
            memcpy(stream, (uint8_t *)is->audio_buf + is->audio_buf_index, len1);
        else {
            // 如果我们调整了音量，就会调 SDL_MixAudioFormat() 来拷贝数据。
    ...
    // isnan---检查是否不是NaN(not a number),不是为true 是为false
    if (!isnan(is->audio_clock)) {
        // 音频流的位置，那就可以反向求音频流当前的播放时刻。 --->is->audio_clock - (double)(2 * is->audio_hw_buf_size + is->audio_write_buf_size) / is->audio_tgt.bytes_per_sec,
...
static int audio_open(void *opaque, AVChannelLayout *wanted_channel_layout, int wanted_sample_rate, struct AudioParams *audio_hw_params)
{
    ...
    const char *env;
    // 其实是一个map表，声道切换映射表。其他声道
    static const int next_nb_channels[] = {0, 0, 1, 6, 2, 6, 4, 6};
    // 采样率，当切换所有声道都无法成功打开音频设备，就需要从 next_sample_rates[] 取一个比当前更小的采样率来尝试。
    ...
    // next_sample_rate_idx 变量，这个变量存的是比 想要打开的音频采样率 更小一点的采样率的索引。
    while (next_sample_rate_idx && next_sample_rates[next_sample_rate_idx] >= wanted_spec.freq)
        next_sample_rate_idx--;
    // 设置打开参数，设置回调函数 ，如下：ffpaly 是写死了采样格式，写死成了 AUDIO_S16SYS 格式。
    wanted_spec.format = AUDIO_S16SYS;
    wanted_spec.silence = 0;
    // wanted_spec.samples 的作用是告诉 SDL 每次回调取多少样本数来播放，实际上就是控制 SDL 每秒调用回调函数的次数。 
    // wanted_spec.freq 是采样率，而 SDL_AUDIO_MAX_CALLBACKS_PER_SEC 代表 SDL 每秒调多少次回调函数。两者相除就能得出每次回调需要读取多少个样本数。
    wanted_spec.samples = FFMAX(SDL_AUDIO_MIN_BUFFER_SIZE, 2 << av_log2(wanted_spec.freq / SDL_AUDIO_MAX_CALLBACKS_PER_SEC));
    // 因为播放采样率是固定的，例如 48000。也就是音频设备每秒要播放 48000 个样本，
    // 如果调一次 sdl_audio_callback 只能取4800个样本，那他一秒内就需要调 10 次 sdl_audio_callback。
...
static int stream_component_open(VideoState *is, int stream_index)
{
    ...
    case AVMEDIA_TYPE_AUDIO:
        ...
        if ((ret = audio_open(is, &ch_layout, sample_rate, &is->audio_tgt)) < 0)
            goto fail;
        // audio_open() 的返回值，最后是赋值给 is->audio_hw_buf_size 的
...
//     char filename[] = "/home/lyt/develop/FFmpeg/Debug/juren-30s.mp4";
